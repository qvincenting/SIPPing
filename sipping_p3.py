...
Via: SIP/2.0/%(transport_mode)s %(source_ip)s:%(source_port)s
Max-Forwards: 70
...
def print_reply(buf, template_vars=None, out_regex=None, out_replace=None, err=None, verbose=False, quiet=False, sock_type=None):
	...
	if sock_type == "UDP":
		...
		template_vars['sock_type'] = sock_type
		try:
			resp = Response(buf.decode())
		except SipUnpackError as e:
			resp = Request(buf.decode())
			...
			try:
				...
				try:
					...
					if options.transport_mode == 'UDP':
						sock_udp.sendto(str(sip_req).encode(),(options.dest_ip, options.dest_port))
					elif options.transport_mode == 'TCP':
						sock_tcp.send(str(sip_req).encode())
					else:
